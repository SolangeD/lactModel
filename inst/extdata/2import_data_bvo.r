...
require("RPostgreSQL")
require("tables")
setwd('D:/ClimGen-CH/Data/Braunvieh3_bvo')
rm(list=ls(all=TRUE))
# Fonction import
#----------------
import_fct=function(extension,type,fin,longueur,columnname,numFiles){
	...
	tablename=paste(extension,"bvo",sep="")
	colnames(data)=columnname #nommer les colonnes pour les avoir dans la BD
	dbWriteTable(con_pos,tablename,value=data.frame(data),row.names=FALSE)

	for(j in 1:length(type)){ #boucle sur les colonnes
		sql1=paste("UPDATE ",tablename," SET ",columnname[j],"=NULL  WHERE ",columnname[j],"=''", sep="") #remplace les '' par des nulls (autrement problème lors du cast)
		sql1b=paste("UPDATE ",tablename," SET ",columnname[j],"=NULL  WHERE ",columnname[j]," like '%#%'", sep="") #remplace les ### par des nulls (autrement problème lors du cast)
		sql1c=paste("UPDATE ",tablename," SET ",columnname[j],"=NULL  WHERE ",columnname[j]," like '-'", sep="") #remplace le - par des nulls (autrement problème lors du cast)
		sql2=paste("ALTER TABLE ",tablename," ALTER ",columnname[j]," TYPE ",type[j]," USING ",columnname[j],"::",type[j], sep="") #cast en type prédéfini
